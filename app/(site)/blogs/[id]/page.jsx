...
function BlogPage() {
  ...
  useEffect(() => {
    async function fetchBlog() {
      ...
      try {
        setLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_BLOG_API}/blog/public/posts/${id}`);
  ...
  if (error || !blog) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4">
        ...
        <p className="text-gray-600 mb-6">{error || "The blog you're looking for doesn't exist or has been removed."}</p>
        <Link href="/blog" className="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
          Back to Blogs
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full h-96 relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4">
          <h1 className="text-3xl md:text-4xl font-bold text-white text-center max-w-4xl">
            {blog.title}
          </h1>
          ...
          {/* Author and Date */}
          <div className="mt-6 text-white/90 text-sm flex items-center">
            {blog.authors && blog.authors.length > 0 ? (
              <span>By {blog.authors.map(author => author.username).join(', ')}</span>
            ) : null}
            {blog.createdAt && (
              <>
